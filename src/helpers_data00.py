...
def ssplit(x):
    ...
    if nr % 2 > 0:
        nrow = int(ceil(x.shape[0] / 2))
        snr = int(ceil(nr / 2))
        p1 = x[snr - 1 : nr,]
    else:
        nrow = int(x.shape[0] / 2)
        snr = int(nr / 2)
        p1 = x[snr:nr,]

    ncol = x.shape[1]
    newarr = np.empty((2, nrow, ncol))
...
def ro_orders(X, *args):
    ...
    Zrow = fc.linkage(clip(X, 0, inf), method="average", metric="euclidean", *args)
    row_order = H.leaves_list(Zrow)
    Zcol = fc.linkage(clip(X, 0, inf).T, method="average", metric="euclidean", *args)
...
def imboard(resp, row_order, col_order, ss_resp_, ypred, alpha_resp, c, filename):
    ...
    xlabel("Dimensions")
    ylabel("Variance")
    text(x=10**1.5, y=10**-2, s=f"alpha={alf}")
